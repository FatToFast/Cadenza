...
**기준 케이던스부터 +10 SPM 안에서 원곡 속도를 우선하고, 필요한 경우에만 1.25배까지 높여 메트로놈으로 실제 케이던스를 보강하는 러닝 전용 음악 플레이어.**
...
> → 원곡 속도로 범위에 들어오는 곡은 그대로, 나머지는 최대 1.25배까지만 조정
> → 범위에 맞지 않는 곡은 자동 건너뛰고, 메트로놈은 현재 곡의 실제 케이던스로 재생
...
> → 원곡 속도 후보가 범위에 있으면 그대로 재생하고, 필요한 경우 1.25배까지만 올림
> → 피치 변경 가능성을 알리고, 범위 밖 곡은 다음 곡으로 안전하게 건너뜀
...
- 원곡 BPM의 1배·2배·4배 케이던스 후보가 허용 구간에 있으면 원곡 속도 1.0배를 우선한다. 예: 92 BPM → 184 SPM.
- 원곡 속도 후보가 없으면 옥타브 폴딩한 음악 목표 BPM으로 기준 케이던스를 맞추되 1.0~1.25배만 허용한다.
- 감속 또는 1.25배 초과가 필요하면 단일 곡은 재생하지 않고, 플레이리스트는 중복 순환 없이 다음 곡으로 이동한다.
...
- 원곡 속도 우선 및 1.0x ~ 1.25x 허용 구간에서 음질 만족스러움
...
- 원곡 속도 1.0배를 우선하고, 속도 조절은 1.0~1.25배만 허용한다. 감속이나 1.25배 초과가 필요한 곡은 재생하지 않는다.
...
- 1.25배를 초과하거나 감속이 필요한 곡: 재생 불가 이유 표시
...
### 9.3 음질 보호를 위한 케이던스 허용 구간

**원곡을 모든 곡에서 정확한 한 케이던스로 강제하면 1.3배 이상의 큰 변속이 자주 필요하고 청취 품질이 무너진다.** 따라서 Cadenza는 정확히 한 숫자를 맞추는 대신 기준부터 +10 SPM을 허용하고 원곡 속도를 우선한다.
...
| 96 | 180 SPM | 필요 1.875배 | 재생 거부/큐 건너뛰기 |
| 120 | 180 SPM | 필요 1.500배 | 재생 거부/큐 건너뛰기 |
...
|---|---|---|
| 1.000배 | 최우선 | `원곡 속도` |
| 1.000~1.100배 | 자연스러운 상향 | 실제 배속 표시 |
| 1.100~1.250배 | 허용 상한 구간 | 실제 배속과 러닝 적합도 표시 |
| 1.250배 초과 | 재생하지 않음 | 단일 곡 차단, 큐 자동 건너뛰기 |
| 1.000배 미만 | 감속하지 않음 | 단일 곡 차단, 큐 자동 건너뛰기 |

Apple Music은 속도 변경 시 피치도 변하므로 1.25배 이내에서도 변화를 느낄 수 있다. 로컬 파일은 `AVAudioUnitTimePitch`로 피치를 유지하지만 같은 상한을 적용해 아티팩트와 장시간 피로를 제한한다.

#### 검증 항목

- [ ] 기본 180에서 90/92/95 BPM이 각각 180/184/190 SPM 원곡 속도로 들리는지 확인
- [ ] 89 BPM이 하한 아래 178 SPM으로 내려가지 않고 약 1.011배로 180 SPM이 되는지 확인
- [ ] 96/120 BPM 단일 곡이 큰 변속 전에 차단되는지 확인
- [ ] 혼합 플레이리스트가 부적합 곡을 건너뛰고 무한 순환하지 않는지 확인
...
| 2026-07-14 | **재생속도는 1.0~1.25배만 허용** | 감속과 큰 상향 변속을 막고 부적합 곡은 단일 차단/큐 건너뛰기로 처리 |
