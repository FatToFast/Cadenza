...
| A-03 | 안전 재생속도 범위 초과 | 기준 케이던스에 맞추려면 1.25배를 초과하거나 감속이 필요함 | "케이던스 범위에 맞지 않는 곡입니다" | 단일 곡은 재생 차단, 큐에서는 다음 재생 가능 곡으로 이동 |
| A-04 | playbackRate 계산 불가 | originalBPM이 0 또는 nil (모든 BPM 소스 실패 + 사용자가 입력 건너뜀) | (내부) rate를 1.0으로 폴백 | 120 BPM 가정값으로 변속하지 않고 확인 상태 유지 + "원곡 BPM을 설정해주세요" 안내. division by zero 절대 방지 |
...
| M-05 | 케이던스 정책 부적합 | 원곡 속도 후보가 허용 구간 밖이고 필요한 배속이 1.25 초과 또는 감속 | "케이던스 범위에 맞지 않는 곡입니다" | 단일 곡은 일시정지, 플레이리스트는 중복 방문 없이 다음 곡 탐색 |
...
**템포 정책**: 곡별 허용 구간은 `기준 케이던스...min(기준+10, 200)`이다. 원곡 BPM의 1배·2배·4배 케이던스 후보가 이 구간에 있으면 원곡 속도(1.0배)를 우선한다. 후보가 없을 때만 기준 케이던스로 상향 조정하며 1.0~1.25배만 허용한다. 원곡 BPM이 유효하지 않으면 120 BPM 가정값으로 변속하지 않고 확인 상태를 유지한다.
...
| 영역 | 테스트 항목 | v0 적용 |
|---|---|---|
| 템포 계획 | 원곡 속도 후보, 상향 +10 허용 구간, 옥타브 폴딩, 1.0~1.25 배속 경계, invalid 입력 | ✅ |
...
| P-07 | 기준 180 + 원곡 96 BPM | 단일 곡 재생 차단 또는 큐 자동 건너뛰기 | v0a |
...
| **playbackRate** | `음악 목표 BPM / 원곡 BPM`. 적용 범위는 1.0~1.25이며 1.0 = 원곡 속도 | 범위 밖 곡은 재생하지 않음 |
