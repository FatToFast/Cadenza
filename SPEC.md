...
|---|---|---|---|---|
| **no_file_selected** | empty state (DESIGN 2.2.1), "음악을 선택하거나 메트로놈만 사용하세요" | 파일 선택, 기준 케이던스 슬라이더, -5/기본/+5, 메트로놈 토글, 재생(메트로놈 전용) | — | — |
| **file_selected_bpm_known** | 곡 제목 + 원곡 BPM + 실제/기준/허용 케이던스 + "키 락 ON" | 전체 컨트롤 | — | `track_loaded`, `bpm_detected` |
| **file_selected_bpm_analyzing** | 곡 제목 + 분석 진행 바 (DESIGN 2.3) | 건너뛰기, 기준 케이던스 슬라이더, 메트로놈 | 건너뛰기 → 탭 템포/수동 입력 | — |
| **analysis_failed** | "BPM을 감지하지 못했습니다" + 재분석/수동 입력 UI | 다시 분석, 수동 입력, 파일 재선택 | 수동 입력 | `analysis_failed` |
| **playback_active** | 곡 정보 + 원곡 BPM + 실제 케이던스 + 재생 중 표시 | 기준 케이던스 슬라이더, -5/기본/+5, 메트로놈 토글/볼륨, 정지 버튼. **파일 선택 비활성** | — | `run_started` |
| **playback_paused** | 곡 정보 + BPM + 일시정지 표시 | 전체 컨트롤 (파일 선택 포함) | — | — |
| **background** | 락스크린 Now Playing + Live Activity: 곡 제목, 실제 케이던스, 기준 케이던스, play/pause | 이어폰 리모트 play/pause | 앱 복귀 후 수동 재개 | — |
...
| **metronome_only** | 메트로놈 단독 모드 (DESIGN 2.2) | 기준 케이던스 슬라이더, -5/기본/+5, 메트로놈 볼륨, 재생(메트로놈), 파일 선택 | — | `run_started` (hasFile=false) |
...
| A-03 | 재생속도 적용 실패 | 오디오 API가 계산된 가속 배속을 적용하지 못함 | "재생속도를 적용할 수 없습니다" | 재생을 멈추고 다시 시도 또는 다른 곡 선택. 1.0배로 조용히 대체하지 않음 |
| A-04 | playbackRate 계산 불가 | originalBPM이 0, nil 또는 30~300 밖임 | "원곡 BPM을 설정해주세요" | 변속·자동 넘김 없이 확인 상태 유지 + 재분석/수동 입력. division by zero 절대 방지 |
...
| B-02 | 자동 분석 실패 | 비트 없는 앰비언트/클래식, 프리뷰 획득 실패 | "BPM을 자동으로 감지하지 못했습니다" | 곡별 자동 시도 1회 후 정지. `다시 분석` 또는 수동 입력 |
| B-03 | 더블/하프 BPM 의심 | 분석 결과가 70~95 구간 (하프) 또는 극단값 | "감지된 BPM: 87. 혹시 174가 맞나요?" | 원곡 BPM은 자동 변경하지 않고 두 선택지 버튼(87 / 174)으로 사용자 확인 |
...
| M-05 | 원곡 BPM 확인 필요 | BPM이 없거나 30~300 밖임 | "원곡 BPM을 설정해주세요" | 변속·템포 기반 자동 넘김 없이 재분석 또는 수동 입력 |
...
|---|---|---|---|
| 원곡 BPM 수동 입력 | 30~300 | 입력 거부 + 빨간 테두리 + "30~300 사이 값을 입력해주세요" | 확인 필요 상태 유지 |
| 기준 케이던스 슬라이더 | 140~200 SPM, 기본 180 | 슬라이더와 모델에서 범위 제한 | N/A |
| 기준 케이던스 빠른 조절 | -5 / 기본 180 / +5 | 140~200에서 클램프 | N/A |

**템포 정책**: 곡별 허용 구간은 `기준 케이던스...min(기준+10, 200)`이다. 30~300 BPM으로 확인된 원곡의 `0.5배·1배·2배·4배` 케이던스 후보가 이 구간에 있으면 원곡 속도(1.0배)를 우선한다. 후보가 없으면 감속하지 않고 2의 거듭제곱 계열에서 원곡 BPM 이상인 가장 작은 음악 목표를 골라 가속한다. 배속 크기는 재생 차단이나 자동 넘김 사유가 아니며, 큰 변화는 `큰 폭 가속`으로 알린다. 원곡 BPM이 없거나 범위 밖이면 변속하지 않고 확인 상태를 유지한다.
...
| 영역 | 테스트 항목 | v0 적용 |
|---|---|---|
| 템포 계획 | `[0.5, 1, 2, 4]` 원곡 속도 후보, 상향 +10 허용 구간, 상위 옥타브 폴딩, 감속 금지, 30~300 경계와 invalid 입력 | ✅ |
...
| P-04 | 재생 중 기준 케이던스 슬라이더 변경 | 즉시 속도 변경, 끊김 없음 | v0a |
| P-05 | 기준 180 + 원곡 92 BPM | 원곡 속도 1.0배, 실제 184 SPM | v0a |
| P-06 | 기준 180 + 원곡 89 BPM | 약 1.011배, 실제 180 SPM | v0a |
| P-07 | 기준 180 + 원곡 96 BPM | 180 BPM 목표, 1.875배, 실제 180 SPM, 차단/자동 넘김 없음 | v0a |
| P-08 | 기준 180 + 원곡 120 BPM | 180 BPM 목표, 1.5배, 실제 180 SPM, `큰 폭 가속` 표시 | v0a |
| P-09 | 곡 끝까지 재생 | 자동 반복 | v0a |
...
| BPM-05 | Apple Music BPM 상태가 미확정 또는 BPM-only | `다시 분석` 노출, 외부 조회와 프리뷰 오디오 분석 재시도 | v1 |
| BPM-06 | 직접 입력한 Apple Music BPM | 재분석 버튼 숨김, 자동 결과가 수동값을 덮어쓰지 않음 | v1 |
...
| R-04 | 위와 동일 | -5/기본/+5 버튼이 한 손으로 탭 가능한가 |
...
| ENG-04 | originalBPM=0 (메타데이터 BPM이 0인 파일) | 확인 필요 상태, A-04 안내, division by zero 없음 | v0a |
| ENG-05 | 메타데이터에 TBPM=0 또는 TBPM=-1인 파일 | 값 무시, 자동 분석 또는 수동 입력으로 진행 | v0a |
| ENG-06 | 온보딩 건너뛰기 → 메인 화면 | empty state 화면 (DESIGN 2.2.1) 정상 표시 | v0a |

### 3.8 테스트 우선순위

**v0a 출시 전 필수**: P-01~P-09, BPM-01, FF-01~FF-05, ENG-01, ENG-03, ENG-04, ENG-05, ENG-06
...
| **기준 케이던스** | 사용자가 조절하는 하한 케이던스. 140~200 SPM, 기본 180 | 곡별 허용 구간의 시작값 |
| **허용 케이던스** | 기준 케이던스부터 최대 +10 SPM까지의 곡별 허용 범위. 전역 상한 200 | 예: 기준 180 → 180~190 |
| **실제 케이던스** | 현재 곡에 실제 적용되는 발걸음 케이던스. 원곡 속도 후보가 허용 범위에 있으면 기준보다 높을 수 있음 | UI와 Live Activity의 대표 SPM |
| **원곡 BPM** | 음악 파일의 원래 템포. 메타데이터 태그, 자동 분석, 탭 템포 등으로 확보 | |
| **음악 목표 BPM** | 케이던스를 음악 박자로 옥타브 폴딩한 내부 계산값. 원곡 BPM 자체를 변경하지 않음 | 예: 기준 180 → 음악 목표 90 BPM 가능 |
| **playbackRate** | `음악 목표 BPM / 원곡 BPM`. 유효 BPM 곡에서는 항상 1.0 이상이며 1.0 = 원곡 속도 | 큰 값은 `큰 폭 가속`으로 표시하되 재생을 막지 않음 |
